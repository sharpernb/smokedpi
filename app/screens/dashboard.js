  ...
  useEffect(() => {
    const connectMQTT = async () => {
      try {
        ...
        // Subscribe to topics - these will now use our new WebSocket format
        subscribeTopic('grill/temp', (message) => {
          // Convert to number if needed - message might be a string or already a number
          const temp = typeof message === 'string' ? Number.parseFloat(message) : message;
          setGrillTemp(temp);
          setTempHistory((prev) => [...prev.slice(1), temp]);
        });

        subscribeTopic('grill/target', (message) => {
          const target = typeof message === 'string' ? Number.parseFloat(message) : message;
          setTargetTemp(target);
        });

        subscribeTopic('grill/fan', (message) => {
          const fan = typeof message === 'string' ? Number.parseFloat(message) : message;
          setFanSpeed(fan);
        });

        subscribeTopic('grill/status/lid', (message) => {
          // Handle different message formats (string "1" or boolean true)
          setLidStatus(message === '1' || message === true || message === 'true');
        });

        subscribeTopic('grill/status/hopper', (message) => {
          setHopperStatus(message === '1' || message === true || message === 'true');
        });

        subscribeTopic('grill/status/ash', (message) => {
          setAshStatus(message === '1' || message === true || message === 'true');
        });

        subscribeTopic('grill/power', (message) => {
          setIsPoweredOn(message === 'true' || message === true);
        });
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>Smoked Pi</Text>
      ...
      <ScrollView style={styles.scrollView}>
        ...
        <View style={styles.actionSection}>
          <TouchableOpacity style={styles.actionButton} onPress={() => router.push('/screens/create-plan')}>
            <Ionicons name="add-circle-outline" size={24} color="#fff" />
            <Text style={styles.actionText}>Create Plan</Text>
          </TouchableOpacity>

          <TouchableOpacity style={styles.actionButton} onPress={() => router.push('/screens/history')}>
            <Ionicons name="analytics-outline" size={24} color="#fff" />
            <Text style={styles.actionText}>History</Text>
          </TouchableOpacity>

          <TouchableOpacity style={styles.actionButton} onPress={() => router.push('/screens/settings')}>
